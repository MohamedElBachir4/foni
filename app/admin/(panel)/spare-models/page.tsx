...
import { Smartphone, CheckCircle, AlertCircle, Pencil, Trash2, Search, ChevronDown, ChevronUp } from "lucide-react";
import {
  AdminButton,
  AdminCard,
  AdminPageHeader,
...
export default function SpareModelsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = { name: name.trim(), image: image.trim(), brand: selectedBrand };
    try {
  ...
  // تجميع الهواتف حسب الماركة
  const groupedByBrand = useMemo(() => {
    const grouped: { [key: string]: { brand: Brand; phones: PhoneType[] } } = {};
    ...
    types.forEach((phone) => {
      ...
      grouped[brandId].phones.push(phone);
    });
